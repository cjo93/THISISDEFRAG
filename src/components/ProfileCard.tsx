...
export default function ProfileCard({
    ...
    return (
        <div className="glass-box rounded-2xl p-8 border border-white/10 relative overflow-hidden group">
            {/* Ambient glow */}
            <div className="absolute -top-20 -right-20 w-60 h-60 bg-slate-500/10 rounded-full blur-3xl opacity-50 group-hover:opacity-70 transition-opacity" />
            ...
            <div className="relative z-10">
                ...
                <div className="flex items-start gap-6 mb-6">
                    ...
                    <div className="relative">
                        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-slate-500/20 to-slate-600/10 border border-slate-500/30 flex items-center justify-center">
                            <span className="text-3xl font-light text-slate-400">{initials}</span>
                        </div>
                        {/* Orbital ring */}
                        <div className="absolute inset-0 rounded-full border border-slate-500/20 animate-[spin_20s_linear_infinite]"
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-slate-400/80 text-sm tracking-wider mb-3">{archetype}</p>
                ...
                <button
                    ...
                    className={`w-full py-4 rounded-xl text-sm tracking-widest uppercase font-medium transition-all flex items-center justify-center gap-3 ${isPlaying
                            ...
                            : isLoading
                                ...
                                : 'bg-white/5 border border-white/10 text-white/70 hover:bg-slate-500/10 hover:border-slate-500/30 hover:text-slate-400'
                        }`}
                >
                    {isLoading ? (
                        <>
                            <div className="w-4 h-4 border-2 border-white/30 border-t-slate-400 rounded-full animate-spin" />
                            Synthesizing...
                        </>
                    ) : isPlaying ? (
                        <>
