...
const parseHorizonsResponse = (response: string, bodyName: string): PlanetaryPosition | null => {
  try {
    // HORIZONS outputs data between $$SOE and $$EOE markers
    ...
    // Parse the first data line - format varies by query type
    // For observer table with ecliptic coords, longitude is in specific column
    const dataLine = lines[0];
    const values = dataLine.trim().split(/\s+/);
    
    // Find ecliptic longitude (ObsEcLon) - typically column index varies
    // We request specific format so longitude should be predictable
    let longitude = 0;
    
    // Try to find a value that looks like ecliptic longitude (0-360)
...
const fetchPlanetaryPosition = async (
  ...
  try {
    // Format date for HORIZONS: 'YYYY-MM-DD HH:MM'
    ...
    // HORIZONS API endpoint (CGI interface)
...
/**
 * Simplified Mars position calculation (fallback)
 */
const calculateMarsPositionFallback = (date: string, time: string): PlanetaryPosition => {
  ...
  // Mars mean longitude (simplified)
 ...
 */
export const calculateMechanics = async (unit: UnitData): Promise<UnitData> => {
  const chart = await calculateNatalChart(unit.birthDate, unit.birthTime);
...
const checkAspects = (transitLong: number, natalLong: number): string[] => {
  ...
  const orb = 8; // Degree orb for aspects
...
export const getFrictionForecast = async (unit: UnitData | null): Promise<FrictionAlert[]> => {
  ...
  try {
    ...
    // Check Mars transits (high voltage warnings)
    ...
    // If no hard aspects, system is nominal
    if (alerts.length === 0) {
      alerts.push({
        level: "🟢 NOMINAL",
        alert: "SYSTEM CLEAR",
...
// Legacy sync wrapper for backwards compatibility during migration
export const calculateMechanicsSync = (unit: UnitData): UnitData => {
  const chart = {
    sun: calculateSunPositionFallback(unit.birthDate, unit.birthTime),
    mars: calculateMarsPositionFallback(unit.birthDate, unit.birthTime)
  };
